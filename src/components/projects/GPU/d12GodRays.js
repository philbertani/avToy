...
// renderer,
let stats,  uvwidth, uvheight, controls, clock;
...
let postprocessing;  //A LOT of stuff gets schlopped into this object
...
export function grInit({renderer ,width,height,version}) {

    //this is what I get for making global variables in a module
    //have to make sure to re initialize
    if (materialDepth.length > 0) {
      materialDepth.forEach(x=>{x.dispose()})
      materialDepth.length = 0
    }

    if (d12Mesh.length > 0) {
      d12Mesh.forEach(x=>{x.geometry.dispose()})
      d12Mesh.length = 0
    }
    ...
    console.log('gr version',version)
    ...
    //return { camera, scene, d12group, d12Mesh, stats ,postprocessing }
    return { camera, scene,  stats }
...
export function renderGR(md,version, renderer) {
    ...
    if ( version === 0) {
      ...
      for (let i = 0; i < numElements; i++) {
        const vv = d12Mesh[i];
        //console.log('iMusic:',iMusic[i%3]*2)
        vv.rotation.y = Math.sin(iMusic[i % 3] * 2) * 3; //.01;
        vv.rotation.z = Math.cos(iMusic[(i + 1) % 3] * 2) * 3;

        tmpSum += Math.abs(vv.rotation.y) + Math.abs(vv.rotation.z)
    ...
    if (postprocessing.enabled) {
        ...
        filterGodRays(renderer, postprocessing.rtTextureDepthMask.texture, postprocessing.rtTextureGodRays2, getStepSize(filterLen, TAPS_PER_PASS, 1.0));

        // pass 2 - render into second ping-pong target
        filterGodRays(renderer, postprocessing.rtTextureGodRays2.texture, postprocessing.rtTextureGodRays1, getStepSize(filterLen, TAPS_PER_PASS, 2.0));

        // pass 3 - 1st RT
        filterGodRays(renderer, postprocessing.rtTextureGodRays1.texture, postprocessing.rtTextureGodRays2, getStepSize(filterLen, TAPS_PER_PASS, 3.0));
...
function filterGodRays(renderer, inputTex, renderTarget, stepSize) {
